...
export async function GET() {
  ...
  if (shouldServeMockData(auth) || !hasSupabaseEnv() || !auth.supabase || !auth.orgId) {
...
export async function POST() {
  ...
  if (auth.profile.role !== 'architect') {
    return NextResponse.json({ error: 'Only architects can create the office project' }, { status: 403 })
  }
