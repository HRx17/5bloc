...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  if (shouldServeMockData(auth)) {
    ...
    const project = MOCK_PROJECTS.find((p) => p.id === invoice.project_id)
    const client = MOCK_CLIENTS.find((c) => c.id === invoice.client_id)
    invoice = {
      ...invoice,
      client_name: client?.full_name || invoice.client_name || 'Client',
      project_name: project?.name || invoice.project_name || 'Project',
    }
    firm = {
      name: auth.profile.organisations?.name || 'Demo Firm',
    ...
    invoice = {
      ...data,
      project_name: data.project_name || data.projects?.name || '—',
    }
    if (auth.orgId) {
