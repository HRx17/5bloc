...
{[
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
  {[
    { icon: 'mail',          iconColor: 'var(--stone)',  title: 'Gmail / Email Sync',        desc: 'Automatically routes coordination digests, RFI queries, and invoice links through your official email address.', status: 'Not connected',   action: 'Set up', onClick: () => toast('Add GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET to .env to enable Gmail OAuth.', 'info', 6000) },
    { icon: 'table_chart',   iconColor: 'var(--stone)',  title: 'Excel / Sheets Automation', desc: 'Enable importing/exporting of BOQs, milestone schedules, and RFI databases. Connect to spreadsheet pipelines.', status: 'Not connected',   action: 'Set up', onClick: () => toast('Google Sheets sync requires an Apps Script webhook — coming soon.', 'info', 6000) },
    { icon: 'calendar_today',iconColor: 'var(--stone)',  title: 'Google Calendar',           desc: 'Synchronizes project milestone dates to your workspace calendar to prevent delayed handovers.', status: 'Not connected',  action: 'Set up',  onClick: () => toast('Add GOOGLE_CLIENT_ID to .env to enable Calendar sync.', 'info', 6000) },
    { icon: 'chat',          iconColor: '#25D366',       title: 'WhatsApp Communication',    desc: 'Send RFI updates and drawings directly to contractors using prefilled mobile links.', status: 'Quick-links ready', action: 'View',   onClick: () => toast('WhatsApp prefilled links work without an API key — no setup needed.', 'success') },
  ].map((card) => (
