...
const handleGET = async ({ request }: any) => {
  ...
  if (!token) return json({ notConnected: true, events: [] })
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return json({ error: `Calendar API error ${res.status}`, detail: errBody }, { status: 502 })
    }
...
const handlePOST = async ({ request }: any) => {
  ...
  const token = await getFreshGoogleToken(user.id)
  if (!token) return json({ error: 'Not connected' }, { status: 403 })
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return json({ error: `Calendar create failed ${res.status}` }, { status: 502 })
    }
...
export const Route = createFileRoute('/api/integrations/google/calendar')({
  server: {
    handlers: {
        GET: handleGET,
        POST: handlePOST,
