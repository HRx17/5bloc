...
const handlePOST = async ({ request }: any) => {
  ...
  const token = await getFreshGoogleToken(user.id)
  if (!token) return json({ error: 'Google not connected' }, { status: 400 })
  ...
  try {
    ...
    return json({ error: 'Failed to save Drive links' }, { status: 500 })
  }
}
...
const handleDELETE = async ({ request }: any) => {
  ...
  try {
    ...
    return json({ error: 'Failed to remove link' }, { status: 500 })
  }
}

export const Route = createFileRoute('/api/integrations/google/drive')({
  server: {
    handlers: {
        GET: handleGET,
        POST: handlePOST,
