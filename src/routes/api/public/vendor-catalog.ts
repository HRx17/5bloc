...
const handlePOST = async ({ request }: any) => {
  try {
    ...
    return json({ error: 'Catalog upload failed' }, { status: 500 })
  }
}
...
async function handleFileUpload(request: Request) {
  ...
  if (file.size > MAX_FILE_MB * 1024 * 1024) {
    return json({ error: `File too large (max ${MAX_FILE_MB} MB)` }, { status: 413 })
  }
...
export const Route = createFileRoute('/api/public/vendor-catalog')({
  server: {
    handlers: {
        POST: handlePOST,
    },
