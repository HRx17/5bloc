...
const handlePOST = async ({ request }: any) => {
  ...
  const { data, error } = await auth.supabase.rpc('accept_project_invite', {
    p_token: token,
    p_full_name: body.full_name || null,
  })
  if (error) return json({ error: error.message }, { status: 500 })
  const result = data as any
  if (!result?.ok) return json({ error: result?.error || 'Could not accept' }, { status: 400 })
  ...
  return json({
    ...
    role,
    redirect: isRoleKey(role) ? homeForRole(role) : `/projects/${result.project_id}`,
  })
}
