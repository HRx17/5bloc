...
export default function ListYourBusinessPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true)
    try {
      const res = await fetch('/api/public/partner/contractor-signup', {
        method: 'POST',
  ...
  return (
    <div className="landing-apple min-h-screen" style={{ background: 'var(--lp-bg)' }}>
      <PartnerSignupHeader secondaryHref="/join-as-vendor" secondaryLabel="Vendor signup" />
      ...
      <main className="mx-auto max-w-[720px] px-5 pb-20 pt-10 sm:px-6">
        {done ? (
          <PartnerSuccess
            ...
            country={country}
            signupHref="/signup?role=contractor"
            signupLabel="Create contractor account"
            steps={[
          ...
          <>
            <PartnerHero
              eyebrow="Contractors · Free listing · India & US"
              title="Join the contractor waitlist."
              body="Get discovered by architects and developers when the marketplace opens. List your trade, service area, and a few project photos — free, no card required."
              bullets={[
                'Free listing — nothing to pay now',
                'Early access before public launch',
                'Architects find you for live projects',
              ]}
            />
            ...
            <form onSubmit={submit} className="grid gap-5">
              ...
              <PartnerSection title="Business details">
                <div className="grid gap-5 sm:grid-cols-2">
                  <PartnerField label="Business name" required>
                    <input {...input} value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="BuildRight Contractors" required />
                  </PartnerField>
                  <PartnerField label="Your name" required>
                    <input {...input} value={contactName} onChange={(e) => setContactName(e.target.value)} placeholder="Amit Sharma" required />
                  </PartnerField>
                </div>
                <div className="grid gap-5 sm:grid-cols-2">
              ...
              <PartnerSection title="What do you do?">
                <PartnerField label="Specializations" required hint="Pick all that apply">
                  <PartnerChipGrid options={SPECIALIZATIONS} selected={specs} onToggle={toggleSpec} />
                </PartnerField>
                <div className="grid gap-5 sm:grid-cols-2">
                  <PartnerField label="Team size">
                  ...
                  <PartnerField label="Years in business">
                    <input {...input} type="number" min="0" value={years} onChange={(e) => setYears(e.target.value)} placeholder="8" />
                  </PartnerField>
                </div>
                <PartnerField label="Website / portfolio">
                  <input {...input} value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://" />
                </PartnerField>
                <PartnerField label="About your work" hint="Strengths, signature projects, certifications.">
                  <textarea
                    {...input}
              ...
              <PartnerSection title="Photos of your work">
                <PartnerField label="Project photos" hint="Optional · up to 6">
                  <div className="flex flex-wrap gap-3">
                    ...
                    {photos.length < 6 && (
                      <button
                        ...
                        className="flex h-24 w-24 flex-col items-center justify-center gap-1.5 rounded-xl transition-colors"
                  ...
                  {photos.some((p) => p.failed) && (
                    <p className="text-[12px] mt-2" style={{ color: '#ff3b30' }}>
                      Some photos couldn&apos;t upload — you can submit without them.
                    </p>
                  )}
                </PartnerField>
              </PartnerSection>

              <PartnerSubmitRow busy={busy} error={error} submitLabel="Join waitlist — free" />
            </form>
          </>
        )}
      </main>

      <PartnerSignupFooter extraLinks={[{ href: '/join-as-vendor', label: 'Vendor signup' }]} />
    </div>
  )
}
