...
function teamInbox() {
  return process.env['SIGNUP_NOTIFY_EMAIL'] || process.env['RESEND_FROM_EMAIL'] || 'contact@5bloc.com'
}
...
/** Confirmation to the user + forward copy to the team inbox. Never throws. */
export async function notifySignup(opts: {
  ...
  const signupUrl = `${appUrl()}/signup${opts.role ? `?role=${encodeURIComponent(opts.role)}` : ''}`
  ...
  if (opts.kind === 'app_account') {
    ...
    userJobs.push(
      safeSend(opts.email, 'You’re on the 5Bloc waitlist', WaitlistConfirmEmail(name, role, signupUrl))
    )
  } else {
