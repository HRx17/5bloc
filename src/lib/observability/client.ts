 ...
 * Loads PostHog only when a PostHog key is set (script inject — no package required).
 */
...
export async function initAnalytics(): Promise<void> {
  ...
  const key = String(env.VITE_POSTHOG_KEY || env.VITE_PUBLIC_POSTHOG_KEY || '').trim()
  if (!key) return

  started = true
  const host = String(env.VITE_POSTHOG_HOST || '').trim() || 'https://us.i.posthog.com'
