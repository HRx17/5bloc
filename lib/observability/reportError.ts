/**
 * Lightweight error reporting — no @sentry/nextjs.
 * Always logs to console; optionally POSTs a Sentry envelope when SENTRY_DSN is set.
 */
...
export async function reportError(err: unknown, context?: ReportContext): Promise<void> {
  ...
  const dsn =
    ...
    (typeof process !== 'undefined' && process.env.SENTRY_DSN?.trim()) ||
    ''
